  ...
  const handleSubscribe = async (planId) => {
    ...
    try {
      if (currentSubscription) {
        ...
        setSuccessMsg('Subscription upgraded successfully rediracting to checkout page!');
        // dispatch(createPaymentLinkThunk({planId, paymentMethod: 'STRIPE'})).unwrap();
      } else {
        // New subscription
        await dispatch(subscribeToPlan({ storeId: store.id, planId })).unwrap();
        setSuccessMsg('Subscribed successfully rediracting to checkout page!');
      }
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans?.map((plan) => (
              <div
                key={plan.id}
                className={`bg-card rounded-2xl p-8 shadow-lg border relative flex flex-col ${currentSubscription?.plan?.id === plan.id ? 'ring-2 ring-primary' : ''
                  }`}
              >
