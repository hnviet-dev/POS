...
export const startShift = createAsyncThunk(
  ...
  async (branchId, { rejectWithValue }) => {
    try {
      // Guard: branchId phải là số hợp lệ
      if (!branchId || isNaN(Number(branchId))) {
        console.error('❌ startShift called with invalid branchId:', branchId);
        return rejectWithValue('Invalid branchId — cannot start shift');
      }

      console.log('🔄 Starting shift...', { branchId });

      const headers = getAuthHeaders();
      const res = await api.post(`/api/shift-reports/start?branchId=${Number(branchId)}`, {}, { headers });
